...
<AccordionGroup>
  ...
  <Accordion title="Other">
    | API |
    |-----|
    | `my.fetchAppInfoListByKeyword` |
    | `my.allowSystemSnapshot` |
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Trigger a payment">
    ```javascript
    // paymentString must come from your backend
    my.tradePay({
...
<Steps>
  ...
  <Step title="Backend creates payment">
    Your backend calls the Rebell Payment API and returns a `paymentString`
  </Step>
  <Step title="Call my.tradePay">
    Pass the `paymentString` to `my.tradePay` — Rebell handles the rest
  </Step>
...
async function initiatePayment(orderData) {
  ...
  const { paymentString } = await response.json();
